...
- Active text output modes: `stdout`, `clipboard`, and `active-app` (Command+V automation).
- `active-app` mode includes clipboard fallback when auto-paste is unavailable.
...
uv run flow-dictate --run-once --simulate-trigger --output clipboard
uv run flow-dictate --run-once --simulate-trigger --output active-app
```
...
- `FLOW_DICTATE_ACTIVE_APP_FALLBACK_TO_CLIPBOARD`: keep clipboard output when active-app paste fails, default `true`.

Notes for `active-app` mode on macOS:
- You may need Accessibility permissions for Terminal (or whichever host app runs `flow-dictate`).
- If automated paste fails, text remains in your clipboard when fallback is enabled.
